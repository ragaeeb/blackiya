...
Claude, Amazon Nova, Meta Muse, and Z.ai rely on page-owned conversation data. If their canonical detail response was not observed, is incomplete, is too large, or has expired, the icon export fails fast instead of guessing a request. Reload or reopen the finished conversation so the platform loads it normally, then try again. Meta Muse caches the classified GraphQL messages archive, joins markdown/JSON artifact iframe bodies when the page has loaded them, and still joins embedded Flight detail with backward pages when those are present; Nova decrypts only the targeted conversation RPC and discards its response-local key; Z.ai combines the conversation detail and message batch before considering the archive eligible.
