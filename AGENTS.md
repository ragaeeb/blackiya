...
- Page-owned cache capture accepts the canonical mapping detail route and the closed `/backend-api/conversations/{id}` flat-message detail route. The flat source `messages` array is retained verbatim while an ordered mapping is built; any response reporting another page is rejected as incomplete.
...
- Meta Muse classifies `POST /api/graphql` by request body. Current complete loads use a messages query with top-level `variables.conversationId` (no `includeMessageList`); the older `id` + `includeMessageList` detail and cursor pagination (`before`/`last`) remain accepted. Title may live only on a separate metadata query, so a closed messages payload is still eligible. Markdown/JSON artifact bodies are loaded in `*.a.metaaiusercontent.com` iframes; a bounded MAIN-world reader posts their Flight source text to the `meta.ai` parent and the assembler joins them by sandbox uuid. Missing artifacts do not fail a ready-terminal export. When Flight still embeds the initial detail, explicit save joins it with captured backward pages. A metadata-only `{ id }` query is not an archive.
...
3. Preserve ChatGPT's complete native `mapping` tree verbatim when supplied. For its closed flat-message detail response, retain the source `messages` array verbatim while building an ordered normalized mapping; reject paginated responses. For other providers without a native mapping, retain the complete canonical provider response in `raw_payload` while building a loss-aware normalized mapping; never export a partial graph as complete.
...
6. Do not persist or cross-world-transfer request-context (provider headers, Gemini batchexecute context) — resolve it in the MAIN-world explicit-action handler, use expiring defensive snapshots, clear stale identity-bearing values when their change is observable, and invalidate the provider snapshot after a `401/403` response. Never infer an account or logout boundary from client metadata, organization/conversation routing, or identifiers inside a conversation response.
