...
- **Meta Muse:** `POST /api/graphql` carries unrelated operations. The interceptor reads Request clones and response clones through the hard byte cap before parsing, and classifies the request body as conversation detail or backward pagination. Eligible detail bodies are the current messages query (`variables.conversationId` without `before`/`last`) and the older `id` + `includeMessageList` query. A metadata-only `{ id }` body and nested `input.conversationId` operations are ignored. Current complete message payloads may omit `title`/`displayTitle`; those fields are optional and the filename falls back to the conversation id when they are absent. Markdown and JSON artifact files are not in the GraphQL body; Meta loads them as iframes on `*.a.metaaiusercontent.com`. A dedicated MAIN-world reader on that host extracts the bounded Next.js Flight source (and recovers JSON from highlighted HTML) and posts it to the `meta.ai` parent. The assembler joins those bodies onto assistant `content.parts` by `html_artifact_sandbox.uuid`. Missing artifacts do not fail a ready-terminal export. When Meta still embeds the initial detail in Next.js Flight data, the explicit save action reads that bounded page-local payload and joins it with captured backward pages. Pagination halves neither invalidate the shared snapshot nor advance provider identity epochs. The assembler uses the same five-minute/12-entry/16-MiB/48-MiB bounds plus a 100-page-per-conversation cap, and joined artifact bodies count toward that shared byte budget; only a closed, ready-terminal archive enters the shared cache.
...
| Meta Muse | Request-body-classified GraphQL archive (closed messages query or cursor-complete detail plus pages), plus joined markdown/JSON artifact iframe bodies when present | No; cache-only | No |
