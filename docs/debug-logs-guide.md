...
- **Meta Muse:** `POST /api/graphql` whose body is the current closed messages query (`variables.conversationId`) or the older `includeMessageList` detail, plus any backward pagination (`before`/`last`). A metadata-only `{ id }` query is not sufficient. Markdown/JSON artifact iframe documents on `*.a.metaaiusercontent.com` are joined by sandbox uuid when present. When Flight still embeds the initial detail, explicit save joins it with captured pages. The assembler requires a closed page range before caching.
