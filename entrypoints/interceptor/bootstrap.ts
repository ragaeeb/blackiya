...
function processGrokFetchStreamChunk(
    ...
    const normalizedChunkText = normalizeGrokStreamChunk(chunkText);
    logGrokFetchChunkProgress(attemptId, conversationId, chunkBytes);
    emitStreamDumpFrame(attemptId, conversationId, 'delta', normalizedChunkText, chunkBytes, 'Grok');
    buffer = appendGrokBufferChunk(buffer, normalizedChunkText);
    ...
    try {
        await consumeReadableStreamChunks(reader, decoder, attemptId, (chunkText) => {
            monitorChatgptSseChunk(chunkText, (chunk) => {
                ...
                sawContent = emitChatGptStreamingLifecycle(attemptId, lifecycleConversationId, sawContent, chunk);
                streamBuffer += chunk;
...
export default defineContentScript({
    ...
    main() {
        ...
        window.fetch = createFetchInterceptor(originalFetch, async (input, init) => {
            const args = [input, init] as Parameters<typeof fetch>;
            return interceptFetchRequest(args);
        });
