import { afterEach, beforeEach, describe, expect, it } from 'bun:test';
import { Window } from 'happy-dom';
import {
    PLATFORM_HEADERS_REQUEST_MESSAGE,
    PLATFORM_HEADERS_RESPONSE_MESSAGE,
    type PlatformHeadersRequestMessage,
} from '@/utils/platform-header-bridge';
import { setSessionToken, stampToken } from '@/utils/protocol/session-token';
import { requestPlatformHeadersFromMainWorld } from '@/features/runtime/platform-header-request';

describe('v3 platform header request', () => {
    let windowInstance: Window;
    let originalWindow: unknown;

    beforeEach(() => {
        windowInstance = new Window();
        originalWindow = (globalThis as any).window;
        (globalThis as any).window = windowInstance;
        setSessionToken('bk:v3-header-request');
    });

    afterEach(() => {
        (globalThis as any).window = originalWindow;
    });

    it('should resolve sanitized headers from the main-world response', async () => {
        windowInstance.addEventListener('message', ((event: MessageEvent) => {
            const request = event.data as PlatformHeadersRequestMessage;
            if (request?.type !== PLATFORM_HEADERS_REQUEST_MESSAGE) {
                return;
            }
            windowInstance.postMessage(
                stampToken({
                    type: PLATFORM_HEADERS_RESPONSE_MESSAGE,
                    requestId: request.requestId,
                    platformName: request.platformName,
                    headers: { authorization: 'Bearer test' },
                }),
                windowInstance.location.origin,
            );
        }) as any);

        await expect(requestPlatformHeadersFromMainWorld('ChatGPT', 100)).resolves.toEqual({
            authorization: 'Bearer test',
        });
    });

    it('should fail fast with undefined when the main world does not respond', async () => {
        await expect(requestPlatformHeadersFromMainWorld('ChatGPT', 5)).resolves.toBeUndefined();
    });
});
