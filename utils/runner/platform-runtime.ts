...
import {
    applyLifecyclePhaseForConversation as applyLifecyclePhaseForConversationCore,
...
import {
    type ButtonHealthCheckDeps,
    type CompletionWatcherDeps,
    type RunnerWindowBridgeDeps,
...
export const runPlatform = (): void => {
    ...
    const buildStreamProbePanelDeps = (): StreamProbePanelDeps => ({
    ...
    function setStreamProbePanel(status: string, body: string) {
        setStreamProbePanelCore(status, body, buildStreamProbePanelDeps());
    }

    function withPreservedLiveMirrorSnapshot(conversationId: string, status: string, primaryBody: string): string {
        return withPreservedLiveMirrorSnapshotCore(streamPreviewState, conversationId, status, primaryBody);
    }

    const buildSyncStreamProbePanelDeps = (): SyncStreamProbePanelDeps => ({
        lastStreamProbeConversationId,
        getAdapterName: () => currentAdapter?.name ?? 'Unknown',
        setStreamProbePanel,
        withPreservedLiveMirrorSnapshot,
    });

    function syncStreamProbePanelFromCanonical(conversationId: string, data: ConversationData) {
        syncStreamProbePanelFromCanonicalCore(conversationId, data, buildSyncStreamProbePanelDeps());
    }

    const appendPendingStreamProbeText = (canonicalAttemptId: string, text: string) => {
        appendPendingStreamProbeTextCore(streamPreviewState, canonicalAttemptId, text, setStreamProbePanel);
    };

    function migratePendingStreamProbeText(conversationId: string, canonicalAttemptId: string) {
        migratePendingStreamProbeTextCore(streamPreviewState, conversationId, canonicalAttemptId, setStreamProbePanel);
    }

    const appendLiveStreamProbeText = (conversationId: string, text: string) => {
        appendLiveStreamProbeTextCore(streamPreviewState, conversationId, text, setStreamProbePanel);
    };
    ...
    // Wire message handlers — delegates to wire-message-handlers module
    ...
    const buildWireMessageHandlerDeps = (): WireMessageHandlerDeps => ({
        ...
        setLifecycleAttemptId: (id) => {
            lifecycleAttemptId = id;
        },
        setLifecycleConversationId: (id) => {
            lifecycleConversationId = id;
        },
        ...
        pendingLifecycleByAttempt,
        sfeUpdateConversationId: (aid, cid) => sfe.getAttemptTracker().updateConversationId(aid, cid),
        refreshButtonState,
        cancelStreamDoneProbe: (aid, reason) => cancelStreamDoneProbe(aid, reason),
        clearCanonicalStabilizationRetry,
        sfeDispose: (aid) => sfe.dispose(aid),
        streamPreviewState,
        attemptByConversation,
    ...
    // Lifecycle phase application — delegates to lifecycle-phase-handler module
    ...
    const buildLifecyclePhaseHandlerDeps = (): LifecyclePhaseHandlerDeps => ({
        ...
        ingestSfeLifecycleFromWirePhase,
        sfeEnabled: () => sfeEnabled,
        sfeResolve: (id) => sfe.resolve(id),
        getCaptureMeta,
        shouldIngestAsCanonicalSample,
        scheduleCanonicalStabilizationRetry,
        runStreamDoneProbe: (cid, aid) => {
            void runStreamDoneProbe(cid, aid);
        },
    });
    ...
    const buildWindowBridgeDeps = (): RunnerWindowBridgeDeps => ({
        ...
        invalidSessionTokenLogAtRef: {
            get value() {
                return lastInvalidSessionTokenLogAt;
            },
            set value(next: number) {
                lastInvalidSessionTokenLogAt = next;
            },
        },
    });
