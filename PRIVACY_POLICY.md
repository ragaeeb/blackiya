...
- a narrow provider-specific allowlist of authentication or client-context headers (including DeepSeek's page-injected bearer authorization), and
...
- **explicit at download time** — observing a response does not create a file. A JSON file is written only after the Blackiya icon is clicked.
...
Some platforms need additional fail-closed assembly before an entry is eligible. Meta Muse joins the initial conversation detail embedded in page-local Next.js Flight data with request-body-classified backward GraphQL pages, in cursor order, and joins markdown/JSON artifact iframe bodies from `*.a.metaaiusercontent.com` by sandbox uuid when the page has loaded them. Amazon Nova accepts only the conversation RPC identified by its target header, decrypts its message strings in memory, and discards the response-local decryption key before caching or export. Z.ai combines identity-consistent conversation detail and message-batch responses. Incomplete, mismatched, oversized, expired, or non-terminal data is not exported.
...
- **`host_permissions`**: Required to run on and communicate with the internal APIs of ChatGPT, Gemini, Grok (`grok.com` and `x.com`), Claude, Amazon Nova, Meta Muse, Qwen, Z.ai, and DeepSeek for explicit conversation export. Meta Muse also needs `https://*.a.metaaiusercontent.com/*` so artifact iframe documents the page already loaded can be read locally and joined into the export; those hosts are not treated as conversation pages and do not show the Blackiya icon. On X, the content scripts are injected across `https://x.com/*` so they are already present when X's SPA navigates from another route into Grok. The Blackiya icon control mounts only on a valid `/i/grok?conversation={id}` route, and conversation-response capture remains limited to the canonical Grok endpoint classifier; unrelated X pages do not show export controls or qualify unrelated responses as conversation archives.
